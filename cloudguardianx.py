...
def analyze_snapshot_health(snapshot_id):
    ...
    if health_score < 0.95:
        message = f"Warning: Snapshot {snapshot_id} health low ({(health_score * 100):.1f}%)"
...
def create_aws_snapshot(volume_id, instance_id=None):
    ...
    try:
        ...
        message = (f"AWS Snapshot {snapshot_id} created (Health: "
                   f"{(health * 100):.1f}%) for Volume {volume_id}")
        send_slack_message(message, SLACK_CHANNEL)
        return snapshot_id
    except Exception as e:
        message = f"Error creating snapshot for Volume {volume_id}: {e}"
        send_slack_message(message, SLACK_CHANNEL)
        print(f"Error: {e}")
        return None
...
def restore_aws_snapshot(snapshot_id, instance_id, volume_id, preview=False):
    if not instance_id or not volume_id:
        ...
        return None
    try:
        instance_data = aws_ec2.describe_instances(
            InstanceIds=[instance_id]
        )['Reservations'][0]['Instances'][0]
        current_volume = instance_data['BlockDeviceMappings'][0]['Ebs']['VolumeId']  # First block device

        if preview:
            message = (f"Preview: Restore {snapshot_id} to Instance {instance_id}, "
                       f"replace Volume {current_volume}")
            send_slack_message(message, SLACK_CHANNEL)
        ...
        aws_ec2.attach_volume(VolumeId=new_volume_id, InstanceId=instance_id,
                             Device='/dev/xvda')

        aws_ec2.start_instances(InstanceIds=[instance_id])
        message = (f"Rollback complete: Instance {instance_id} restored to Volume "
                   f"{new_volume_id}")
        send_slack_message(message, SLACK_CHANNEL)
        return new_volume_id
    except Exception as e:
        message = (f"Rollback error for Instance {instance_id}, Volume {volume_id}: "
                   f"{e}")
        send_slack_message(message, SLACK_CHANNEL)
        print(f"Error: {e}")
        return None
...
def delete_old_aws_snapshots(volume_id=None, days=30):
    ...
    for snap in snapshots:
        age = (datetime.now() - datetime.fromisoformat(
            snap['StartTime'].replace('Z', '+00:00')
        )).days
        if age > days:
            aws_ec2.delete_snapshot(SnapshotId=snap['SnapshotId'])
            message = (f"Deleted old snapshot {snap['SnapshotId']} for Volume "
                       f"{volume_id or 'all'}")
            send_slack_message(message, SLACK_CHANNEL)
...
def gamified_recovery_challenge(snapshot_id, instance_id, volume_id):
    ...
    if new_volume:
        ...
        message = (f"Challenge: Restored in {recovery_time:.1f}s for Instance "
                   f"{instance_id}. Score: {score}/100")
        send_slack_message(message, SLACK_CHANNEL)
        return score
    return 0
